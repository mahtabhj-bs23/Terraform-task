...
data "template_file" "tsintegration-celery-tmplate" {
  template = file("${path.module}/templates/tsintegration-celery.json.tpl")
...
resource "aws_ecs_task_definition" "task_station_integration_celery_task_defination" {
  family                   = "task_station_integration_celery_task_defination"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = "256"
  memory                   = "512"
  execution_role_arn       = aws_iam_role.ecs_task_execution_role.arn
  task_role_arn            = aws_iam_role.ecs_task_role.arn
  container_definitions    = data.template_file.tsintegration-celery-tmplate.rendered
}
